...
* **이번 달 사용자별 추정 데이터 소비량**:
  * 이번 달 1일 00:00부터 현재까지 각 사용자 계정별로 발생한 네트워크 전송량(GB/TB)을 정밀 계산하여 순위별로 정렬합니다.
  * 월간 총 전송량, 외부(WAN) 전송량, 내부(LAN) 전송량, 활성 사용자 수 및 1인당 평균 소비량을 한눈에 확인할 수 있습니다.
  * 사용자 닉네임(별칭) 설정이 연동되어 알아보기 쉬운 이름으로 표시되며, 전체 트래픽 중 개인별 점유율(%) 프로그레스 바와 WAN/LAN 상세치를 제공합니다.
